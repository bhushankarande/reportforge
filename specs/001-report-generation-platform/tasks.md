...
- [x] 0.1 ModelRouter: tools/llm/model_router.py — reads ACTIVE_LLM_PROVIDER, returns correct AgentScope wrapper for nvidia/groq/ollama.
- [x] 0.2 QuotaManager: tools/llm/quota_manager.py — SQLite-backed counters. Enforces NVIDIA NIM RPM and Groq request/token limits. Raises graceful QuotaExceededError.
- [x] 0.3 CostEstimator: tools/llm/cost_estimator.py — computes actual $0.00 for free-tier calls. Pure pricing logic, no side effects.
- [x] 0.4 Configuration: app/config.py — pydantic-settings with all provider API keys, quotas, cost guards, and ACTIVE_LLM_PROVIDER defaulting to nvidia.
...
- [x] 3.1 Planner Agent: agents/planner_agent.py — topic → PlannerOutput (NVIDIA NIM free endpoint) [P]
- [x] 3.2 Research Agent: agents/research_agent.py — mock search → ResearchOutput (NVIDIA NIM free endpoint) [P]
...
- [x] 3.5 Report Writer Agent: agents/writer_agent.py — WriterInput → WriterOutput (NVIDIA NIM, section-by-section)
- [x] 3.6 Verifier Agent: agents/verifier_agent.py — claims → VerifierOutput (NVIDIA NIM free endpoint)
- [x] 3.7 Critic Agent: agents/critic_agent.py — full report → CriticOutput (NVIDIA NIM free endpoint)
...
- [x] 4.3 Cost tracking wrapper: orchestration/cost_tracking.py — CostTrackingModel decorator for AgentScope. Calls tools/llm/cost_estimator.py for math. Logs $0.00 actual cost.
...
- [x] 5.5 Cost endpoints: app/routers/costs.py — summary dashboard showing free-tier $0.00 cost
- [x] 5.6 Streamlit frontend: frontend/streamlit_app.py — upload, progress, source panel, export, provider selector (NVIDIA/Groq/Ollama)
...
- [x] 6.4 Unit tests: tests/test_*.py covering every agent and tool with mocked LLM calls
...
- [x] 6.8 End-to-end test: one full report generation from topic to PDF using NVIDIA NIM
